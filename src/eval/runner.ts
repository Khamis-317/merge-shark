...
export async function runEvaluation(options: {
  ...
  if (options.mode === 'full-repo' && !adapter.supports.fullRepo) {
    throw new Error(
      `Dataset ${adapter.name} does not support full-repo evaluation.`
    );
  }

  if (
    options.mode === 'snippet' &&
    (dataset === 'pull-request-conflicts' || dataset === 'local-conflict-repos')
  ) {
    throw new Error(
      `${dataset} does not include snippet ground-truth resolutions; use full-repo mode.`
    );
  }
  ...
  for await (const evalCase of cases) {
    ...
    try {
      harnessRes =
        options.mode === 'full-repo'
          ? await runFullRepoMode(evalCase, {
              agent,
    ...
    const syntax =
      options.mode === 'full-repo' && evalCase.repoPath
    ...
    if (options.judgeModel !== undefined) {
      try {
        semantic = await evaluateSemanticQuality(
          evalCase,
          harnessRes.resolution,
          options.judgeModel
        );
      } catch (e) {
    ...
    if (adapter.supports.ragTracking) {
      const expectedFiles = await resolveExpectedFiles(evalCase, {
    ...
    const deterministicScore =
      options.mode === 'full-repo'
        ...
        : (syntax.markersClean ? SCORE_WEIGHT_SNIPPET_MARKERS : 0) +
          similarity.score * SCORE_WEIGHT_SNIPPET_SIMILARITY;
    const qualityScore = semantic
      ? deterministicScore * SCORE_WEIGHT_OVERALL_DETERMINISTIC +
        semantic.score * SCORE_WEIGHT_OVERALL_SEMANTIC
      : deterministicScore;
    ...
    const result: EvalResult = {
      ...
      case: {
        ...
        ...(evalCase.conflictType
          ? { conflictType: evalCase.conflictType }
          : {}),
        conflictText: evalCase.conflictText,
        ...(evalCase.conflictContext
          ? { conflictContext: evalCase.conflictContext }
          : {}),
...
async function evaluateFullRepoSyntax(repoPath: string): Promise<SyntaxResult> {
  ...
  return {
    ...
    errors: unresolvedFiles.map(
      (file) => `Unresolved conflict markers remain in ${file}`
    ),
  };
}
...
async function cleanupWorktree(evalCase: {
