...
function slWindow(
  hashes: { hash: number; index: number }[],
  n: number
...
function getMin(
  windows: { hash: number; index: number }[][]
): { hash: number; index: number }[] {
  const result: { hash: number; index: number }[] = [];
  ...
  for (const w of windows) {
    ...
    for (const h of w) {
      if (
        h.hash < minH.hash ||
        (h.hash === minH.hash && h.index > minH.index)
      ) {
        minH = h;
      }
    }
    ...
    if (
      !prevMin ||
      minH.hash !== prevMin.hash ||
      minH.index !== prevMin.index
    ) {
      result.push(minH);
    }
    prevMin = minH;
...
export function winnowing(genStr: string, standardStr: string): number {
  ...
  const winnow = (text: string) => {
    ...
    if (hashes.length < winSize)
      return new Set<number>(hashes.map((h) => h.hash));
  ...
  const intersect =
    hashA.filter((v) => w2.has(v)).length +
    hashB.filter((v) => w1.has(v)).length;
...
export function lineDiffOverlap(genStr: string, standardStr: string): number {
  const genLines = genStr
    .split('\n')
    .map((s) => s.trim())
    .filter(Boolean);
  const stdLines = standardStr
    .split('\n')
    .map((s) => s.trim())
    .filter(Boolean);
...
export function evaluateSimilarity(
  ...
  const score = em
    ? 1.0
    : SCORE_WEIGHT_EDIT_DISTANCE * ed +
      SCORE_WEIGHT_WINNOWING * win +
      SCORE_WEIGHT_LINE_DIFF * ld;
