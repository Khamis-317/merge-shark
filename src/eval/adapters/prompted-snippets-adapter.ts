  ...
  async *load(options: AdapterOptions): AsyncIterable<EvalCase> {
    ...
    const langDirs = entries
      .filter(
        (e) =>
          e.isDirectory() &&
          (e.name.startsWith('repos_github_') ||
            e.name.startsWith('repos_reaper_'))
      )
      .map((e) => e.name);
    ...
    for (const langDir of langDirs) {
      ...
      try {
        ...
        console.warn(
          `Could not read prompted snippet test directory ${testDir}: ${formatError(error)}`
        );
        continue;
      }
...
async function findPromptedSnippetsPath(
  datasetPath: string | undefined
): Promise<string> {
  const basePath = path.resolve(datasetPath ?? DEFAULT_EVAL_DATASETS_DIR);
  const candidates = [basePath, path.join(basePath, 'merges')];
    ...
    try {
      ...
      if (
        entries.some(
          (entry) =>
            entry.isDirectory() &&
            (entry.name.startsWith('repos_github_') ||
              entry.name.startsWith('repos_reaper_'))
        )
      ) {
        return candidate;
      }
      ...
      if (!isNotFoundError(error)) {
        console.warn(
          `Could not inspect prompted snippet data path ${candidate}: ${formatError(error)}`
        );
      }
      continue;
    }
  ...
  throw new Error(
    `Prompted snippet data not found. Expected repos_github_* or repos_reaper_* directories under one of: ${candidates.join(', ')}`
  );
}
